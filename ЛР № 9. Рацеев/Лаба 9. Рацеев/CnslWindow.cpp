...
public:
	void ConsoleWindowProc()
	{
		...
		CONSOLE_SCREEN_BUFFER_INFO csbi; // получение информации из буфера окна
		...
		GetConsoleScreenBufferInfo(out_handle, &csbi); // получение размера буфера от дескриптора стандартного.
		csbi.dwSize.Y = 10000;//10000 строк
		SetConsoleScreenBufferSize(out_handle, csbi.dwSize); // установить количество указанных строк в dwSize.Y
	}
};
